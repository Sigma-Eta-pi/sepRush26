...
const router = Router();
const VALID_EVENT_TYPES = ['general', 'social', 'professional', 'exec', 'mandatory'];
...
router.post('/', requireExec, async (req, res) => {
  ...
  if (!date || (typeof date === 'string' && !DATE_RE.test(date) && isNaN(Date.parse(date)))) { res.status(400).json({ error: 'Valid date is required (YYYY-MM-DD)' }); return; }
  if (type && !VALID_EVENT_TYPES.includes(type)) { res.status(400).json({ error: `Type must be one of: ${VALID_EVENT_TYPES.join(', ')}` }); return; }
  ...
  res.json({ id, title, description, date, time, location, type: type || 'general', createdBy: req.user!.id, createdAt: now });
});

router.put('/:id', requireExec, async (req, res) => {
  const rows = await sql`SELECT * FROM events WHERE id = ${req.params.id} LIMIT 1`;
  if (rows.length === 0) { res.status(404).json({ error: 'Not found' }); return; }
  const e = rows[0];

  const { title, description, date, time, location, type } = req.body;
  if (date !== undefined && typeof date === 'string' && !DATE_RE.test(date) && isNaN(Date.parse(date))) { res.status(400).json({ error: 'Valid date is required (YYYY-MM-DD)' }); return; }
  if (type !== undefined && !VALID_EVENT_TYPES.includes(type)) { res.status(400).json({ error: `Type must be one of: ${VALID_EVENT_TYPES.join(', ')}` }); return; }
  ...
  `;
  const updated = await sql`SELECT * FROM events WHERE id = ${req.params.id} LIMIT 1`;
