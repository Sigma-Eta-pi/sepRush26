...
router.get("/", requireAuth, async (req, res) => {
  ...
  res.json(
    rows.map(t => ({
      ...
      assignedToName:
        t.assigned_to_name || t.assigned_to_email || t.assigned_to,
...
router.post("/", requireExec, async (req, res) => {
  ...
  if (!title || !assignedTo) {
    res.status(400).json({ error: "Title and assignedTo required" });
    return;
  }
...
router.put("/:id", requireAuth, async (req, res) => {
  const user = req.user!;
  const rows =
    await sql`SELECT * FROM tasks WHERE id = ${req.params.id} LIMIT 1`;
  if (rows.length === 0) {
