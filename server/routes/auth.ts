...
router.post("/register", requireAdmin, async (req, res) => {
  ...
  try {
    await sendWelcomeEmail(email.trim(), token);
  } catch (e) {
    console.error("Failed to send welcome email:", e);
  }

  res.json({
    user: { id, email: email.trim(), role, createdAt: now },
    inviteSent: true,
  });
});
...
router.post("/reset-password", async (req, res) => {
  ...
  const rows =
    await sql`SELECT * FROM password_reset_tokens WHERE token = ${token} AND used = 0 LIMIT 1`;
  if (rows.length === 0) {
    res.status(400).json({ error: "Invalid or expired link" });
    return;
  }
  ...
  const userRows =
    await sql`SELECT id, email, role FROM users WHERE id = ${rows[0].user_id} LIMIT 1`;
