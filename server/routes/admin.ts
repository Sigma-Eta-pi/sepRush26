...
router.put("/users/:id", requireAdmin, async (req, res) => {
  const rows =
    await sql`SELECT id, email, role FROM users WHERE id = ${req.params.id} LIMIT 1`;
  if (rows.length === 0) {
  ...
  if (email && email !== u.email) {
    const conflict =
      await sql`SELECT id FROM users WHERE email = ${email} AND id != ${req.params.id} LIMIT 1`;
...
router.post("/users/:id/reset-password", requireAdmin, async (req, res) => {
  const rows =
    await sql`SELECT id, email FROM users WHERE id = ${req.params.id} LIMIT 1`;
  if (rows.length === 0) {
    res.status(404).json({ error: "User not found" });
    return;
  }
...
router.post("/email-blast", requireExec, async (req, res) => {
  ...
  const user = req.user!;
  const senderRows =
    await sql`SELECT name FROM profiles WHERE user_id = ${user.id} LIMIT 1`;
  ...
  if (emails.length === 0) {
    res.json({ success: true, sent: 0, message: "No active members to email" });
    return;
  }
  ...
  try {
    ...
    res.json({
      success: true,
      sent: emails.length,
      message: `Email sent to ${emails.length} active member${emails.length === 1 ? "" : "s"}`,
    });
  } catch (e: any) {
