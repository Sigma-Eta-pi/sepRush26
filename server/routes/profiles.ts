...
router.get("/:userId", requireAuth, async (req, res) => {
  const rows =
    await sql`SELECT * FROM profiles WHERE user_id = ${req.params.userId} LIMIT 1`;
  if (rows.length === 0) {
...
router.post("/", requireAuth, async (req, res) => {
  ...
  const photoUrl =
    typeof req.body.photoUrl === "string" ? req.body.photoUrl : undefined;
  const gradYear = req.body.gradYear ?? null;
  const userId = req.user!.id;
  const now = new Date().toISOString();

  const existing =
    await sql`SELECT * FROM profiles WHERE user_id = ${userId} LIMIT 1`;
...
router.put("/:id", requireAuth, async (req, res) => {
  const rows =
    await sql`SELECT * FROM profiles WHERE id = ${req.params.id} LIMIT 1`;
  if (rows.length === 0) {
    res.status(404).json({ error: "Profile not found" });
    return;
  }
