...
router.post("/", requireExec, async (req, res) => {
  ...
  const content =
    typeof req.body.content === "string"
      ? req.body.content.trim().slice(0, 5000)
      : "";
  if (!title) {
  ...
  const profileRows =
    await sql`SELECT name FROM profiles WHERE user_id = ${req.user!.id} LIMIT 1`;
...
router.put("/:id", requireExec, async (req, res) => {
  const rows =
    await sql`SELECT * FROM updates WHERE id = ${req.params.id} LIMIT 1`;
  if (rows.length === 0) {
