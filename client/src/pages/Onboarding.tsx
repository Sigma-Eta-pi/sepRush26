...
function CustomSelect({
  ...
  return (
    <div ref={ref} className={`relative ${className ?? ""}`}>
      ...
      >
        <span className={selected ? "" : "text-[#EEEADE]/30"}>
          {selected ? selected.label : (placeholder ?? "Select...")}
        </span>
        <ChevronDown
          size={16}
          className={`text-[#EEEADE]/50 transition-transform shrink-0 ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && (
          ...
          {options.map(o => (
            <button
              ...
              className={`w-full px-4 py-2.5 text-left text-sm transition-colors flex items-center justify-between ${
                value === o.value
                  ? "bg-white/20 text-[#EEEADE]"
                  : "text-[#EEEADE]/70 hover:bg-white/10 hover:text-[#EEEADE]"
              }`}
            >
              {o.label}
              {value === o.value && (
                <Check size={13} className="text-[#EEEADE]" />
              )}
            </button>
          ))}
        </div>
...
export default function Onboarding() {
  ...
  const inputCls =
    "w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-[#EEEADE] placeholder:text-[#EEEADE]/30 focus:outline-none focus:border-[#EEEADE]/50 text-sm transition-colors";
            ...
            <div key={s.id} className="flex items-center gap-2">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                  i < step
                    ? "bg-[#EEEADE] text-[#05006C]"
                    : i === step
                      ? "bg-white/20 border-2 border-[#EEEADE] text-[#EEEADE]"
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={`h-px w-6 transition-all ${i < step ? "bg-[#EEEADE]" : "bg-white/20"}`}
                />
              )}
            </div>
          ))}
        ...
        <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl overflow-hidden">
          <AnimatePresence custom={direction} mode="wait">
            ...
            >
              ...
              {currentStepId === "password" && (
                <div>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-[#EEEADE] font-bold text-lg tracking-wide">
                      Set Your Password
                    </h2>
                  </div>
                  <p className="text-[#EEEADE]/50 text-sm mb-6">
                    You're using a temporary password. Choose a personal one to
                    secure your account.
                  </p>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-[#EEEADE]/60 text-xs uppercase tracking-wider mb-1.5">
                        New Password
                      </label>
                      <input
                        type="password"
              ...
              {currentStepId === "welcome" && (
                <div className="text-center">
                  ...
                  <p className="text-[#EEEADE]/60 text-sm leading-relaxed mb-2">
                    Hey {user?.email?.split("@")[0]}, let's set up your profile
                    so your brothers can get to know you.
                  </p>
                  <p className="text-[#EEEADE]/40 text-xs">
              ...
              {currentStepId === "basics" && (
                <div>
                  <h2 className="text-[#EEEADE] font-bold text-lg tracking-wide mb-6">
                    Your Info
                  </h2>
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {linkedinPhoto && (
                          <img
                            src={linkedinPhoto}
                            alt="LinkedIn photo"
                            className="w-10 h-10 rounded-full object-cover border-2 border-green-400 shrink-0"
                          />
                        )}
                      </div>
                      {linkedinPhoto && (
                        <p className="text-green-400 text-xs mt-1">
                          Profile photo found — will be used on your member
                          card.
                        </p>
                      )}
                    </div>
          ...
          <div className="px-8 pb-8 flex items-center justify-between">
            {step > 0 && currentStepId !== "password" ? (
              <button
                onClick={goPrev}
                className="flex items-center gap-1.5 text-[#EEEADE]/50 hover:text-[#EEEADE] text-sm transition-colors"
              >
            ...
            {!isLast ? (
              ...
              >
                {submitting ? (
                  ...
                  <>
                    {currentStepId === "password" ? "Set Password" : "Next"}{" "}
