  ...
  const isActive = (href: string) => {
    if (href === "/dashboard/updates") {
      return (
        location === "/dashboard" ||
        location === "/dashboard/" ||
        location.startsWith("/dashboard/updates")
      );
    }
    ...
    switch (path) {
      ...
      case "admin":
        return (
          <Suspense
            fallback={
              <div className="text-[#05006C]/50">Loading admin panel...</div>
            }
          >
            <AdminPanel />
          </Suspense>
      ...
      <div className="fixed left-0 top-0 w-64 h-full bg-[#05006C] flex flex-col">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <div className="text-[#EEEADE] font-bold text-sm tracking-widest">
                SIGMA ETA PI
              </div>
              <div className="text-[#EEEADE]/50 text-xs tracking-wider">
                EPSILON CHAPTER
              </div>
            </div>
          </div>
        ...
        <div className="px-4 pb-4 border-t border-white/10 pt-4">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <div className="text-[#EEEADE]/60 text-xs truncate">
                {user?.email}
              </div>
              <span
                className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${roleBadgeColor}`}
              >
                {user?.role}
              </span>
            </div>
